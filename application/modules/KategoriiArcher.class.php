...
class KategoriiArcher extends modules {
	...
	function __construct() {
		if(defined("IS_ADMINCP")) {
			...
			KernelArcher::callback("ShieldFunc", "KategoriiArcher::RebuildShield");
	...
	public function RebuildShields($table, $page, $model, $tpl) {
		if($model->loadedTable()=="cardinal_kategorii") {
			$model->setAttribute('kKategoriya', 'Type', 'varchar');
		}
	...
	public static function RebuildEditModel($model, &$exc = array()) {
		if($model->loadedTable()=="cardinal_kategorii") {
			$model->setAttribute('kKategoriya', 'default', '');
$model->setAttribute('kKategoriya', 'height', 'auto');
$model->setAttribute('kKategoriya', 'Type', 'varchar');
		}
		return array($model);
	}
