...
class modules {
	...
	final public static function regCssJs($js, $type, $mark = false, $name = "", $id = false) {
		global $manifest;
		$jsCheck1 = false;
		...
		if(is_array($js) && !isset($js['url'])) {
			foreach($js as $k => $v) {
				self::regCssJs($v, $type, $mark, (is_numeric($k) ? $name : $k), $id);
			}
		} else {
			...
			$manifest['jscss'][$type][$typeData][$id] = array("name" => $name, "url" => $link, "defer" => (isset($js['defer']) && $js['defer']==true ? true : false));
