...
function esc_cardinal($text) {
		...
		if(!$is_utf8) {
			...
			if(!$utf8_pcre) {
				...
				if(@preg_match('/^./us', $text)===1) {
					$safe_text = $text;
				} else if($strip && function_exists('iconv')) {
	...
	if(strlen($safe_text)===0) {
		...
		if(preg_match('/[&<>"\']/', $safe_text)) {
			...
			$safe_text = preg_replace_callback('/&amp;([A-Za-z]{2,8}[0-9]{0,2});/', function($m) {
				if(empty($matches[1])) {
					return '';
				}
				$i = $matches[1];
			...
			$safe_text = @htmlspecialchars($safe_text, $quote_style, $charset, $double_encode);
		}
	}
	return $safe_text;
}
...
function esc_url($url, $protocols = null) {
	...
	// display
		$url = str_replace('&', '&amp;', $url);
		$url = preg_replace_callback('/&amp;([A-Za-z]{2,8}[0-9]{0,2});/', function($m) {
			if(empty($matches[1])) {
				return '';
			}
			$i = $matches[1];
