...
class Archer extends Core {
	...
	function findDuplicate($v, $list, $creator, $uniq) {
		$counter = db::doquery("SELECT COUNT(".key($list).") AS `id` FROM {{".$creator."}} WHERE `".$uniq."` LIKE ".db::Escape($v));
		if($counter['id']>0) {
			$v .= "_".$counter['id'];
			$v = $this->findDuplicate($v, $list, $creator, $uniq);
		}
		return $v;
	}
	...
	function __construct() {
		$request = new Request();
		execEventRef("request-archer", $request);
